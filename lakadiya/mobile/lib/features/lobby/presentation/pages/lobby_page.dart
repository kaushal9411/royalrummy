...
class _LobbyPageState extends State<LobbyPage> with TickerProviderStateMixin {
  ...
  Future<void> _joinByCode() async {
    ...
    } catch (_) { _showError('Room not found or full'); }
    finally { if (mounted) setState(() => _loading = false); }
  }
            ...
            child: RefreshIndicator(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    _WelcomeBanner(username: username, level: level, coins: coins, xp: xp),
                    const SizedBox(height: 24),
                    _SectionLabel(title: 'Quick Play', icon: Icons.flash_on_rounded),
                    ...
                    _JoinCodeCard(ctl: _codeCtl, loading: _loading, onJoin: _joinByCode),
                    const SizedBox(height: 24),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        _SectionLabel(title: 'Open Rooms', icon: Icons.meeting_room_rounded),
                        GestureDetector(
                    ...
                    if (_publicRooms.isEmpty)
                      ...
                      ...List.generate(_publicRooms.length, (i) => _RoomCard(
                        room: _publicRooms[i], index: i,
                        onJoin: () async {
                          setState(() => _loading = true);
                          try {
                          ...
                          } catch (_) { _showError('Could not join room'); }
                          finally { if (mounted) setState(() => _loading = false); }
                        },
                      )),
                    const SizedBox(height: 24),
        ...
        Container(
          decoration: const BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: [Color(0xFF060D1A), Color(0xFF08121E), Color(0xFF050A13)],
              stops: [0, 0.5, 1],
            ),
          ),
        ),
...
class _WelcomeBanner extends StatelessWidget {
  ...
  const _WelcomeBanner({required this.username, required this.level, required this.coins, required this.xp});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.primary.withValues(alpha: 0.10), blurRadius: 20, offset: const Offset(0, 6))],
      ),
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(
            ...
            children: [
              const Text('Welcome back', style: TextStyle(color: AppColors.textSecondary, fontSize: 11)),
              Text(username,
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    Text(title, style: const TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 17)),
  ]);
}
...
class _ActionCard extends StatefulWidget {
  ...
  const _ActionCard({required this.suit, required this.label, required this.subtitle, required this.colors, this.onTap});
  @override
    ...
    child: ScaleTransition(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            begin: Alignment.topLeft, end: Alignment.bottomRight,
            colors: [widget.colors[0].withValues(alpha: 0.18), widget.colors[1].withValues(alpha: 0.08)],
          ),
          border: Border.all(color: widget.colors[0].withValues(alpha: 0.38)),
          boxShadow: [BoxShadow(color: widget.colors[0].withValues(alpha: 0.14), blurRadius: 14, offset: const Offset(0, 5))],
        ),
        child: Stack(children: [
          Positioned(
            right: -4, bottom: -10,
            child: Text(widget.suit,
                style: TextStyle(color: widget.colors[0].withValues(alpha: 0.09), fontSize: 64, fontWeight: FontWeight.bold)),
          ),
          Column(children: [
            Container(
              width: 54, height: 54,
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: widget.colors[0].withValues(alpha: 0.5), blurRadius: 12)],
              ),
              child: Center(child: Text(widget.suit,
                  style: const TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.bold))),
            ),
            const SizedBox(height: 12),
            Text(widget.label, style: TextStyle(color: widget.colors[0], fontWeight: FontWeight.bold, fontSize: 15)),
...
class _JoinCodeCard extends StatelessWidget {
  ...
  const _JoinCodeCard({required this.ctl, required this.loading, required this.onJoin});
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      boxShadow: [BoxShadow(color: AppColors.trump.withValues(alpha: 0.08), blurRadius: 14, offset: const Offset(0, 5))],
    ),
    child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Row(children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(colors: [AppColors.trump, AppColors.trump.withValues(alpha: 0.7)]),
        ...
        const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Join with Code', style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 16)),
          Text('Enter room code to join friends', style: TextStyle(color: AppColors.textMuted, fontSize: 11)),
        ]),
      ]),
      const SizedBox(height: 14),
      Row(children: [
        Expanded(child: TextField(
          ...
          style: const TextStyle(color: AppColors.accent, fontWeight: FontWeight.bold, fontSize: 20, letterSpacing: 6),
          decoration: InputDecoration(
            hintText: '• • • • • •',
            hintStyle: TextStyle(color: AppColors.textMuted.withValues(alpha: 0.4), letterSpacing: 4, fontSize: 16),
            filled: true,
            ...
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: AppColors.trump, width: 2)),
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(colors: loading
                  ...
                  : [AppColors.trump, AppColors.trump.withValues(alpha: 0.8)]),
              borderRadius: BorderRadius.circular(12),
              boxShadow: loading ? null : [BoxShadow(color: AppColors.trump.withValues(alpha: 0.4), blurRadius: 8)],
            ),
            child: const Text('Join', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
          ),
        ),
      ]),
    ]),
...
class _EmptyRoomsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(children: [
      ...
      const SizedBox(height: 14),
      const Text('No open rooms', style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 16)),
      const SizedBox(height: 4),
      const Text('Create one and invite friends!', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
    ]),
  );
}
      ...
      child: ScaleTransition(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(20),
            gradient: disabled
                ? const LinearGradient(colors: [Color(0xFF1A2A1A), Color(0xFF101A10)])
...
class _RoomCard extends StatelessWidget {
  ...
  final int index;
  final VoidCallback onJoin;
  const _RoomCard({required this.room, required this.index, required this.onJoin});
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder<double>(
      ...
      builder: (_, v, child) => Opacity(
        opacity: v, child: Transform.translate(offset: Offset(0, 20 * (1 - v)), child: child)),
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: isFull ? AppColors.darkBorder : AppColors.primary.withValues(alpha: 0.22)),
          boxShadow: [BoxShadow(
            color: isFull ? Colors.transparent : AppColors.primary.withValues(alpha: 0.07),
        ...
        child: Row(children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: isFull ? AppColors.darkBorder : AppColors.primary.withValues(alpha: 0.4)),
            ),
            child: Center(child: Text(
              host.isNotEmpty ? host[0].toUpperCase() : '?',
              style: TextStyle(color: isFull ? AppColors.textMuted : Colors.white, fontWeight: FontWeight.bold, fontSize: 18),
            )),
          ),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(host, style: const TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.bold, fontSize: 14)),
            ...
            Row(children: [
              ...
              ...List.generate(4, (i) => Container(
                ...
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  color: i < count ? AppColors.primary : AppColors.darkBorder,
                  boxShadow: i < count ? [BoxShadow(color: AppColors.primary.withValues(alpha: 0.5), blurRadius: 4)] : null,
                ),
              )),
              const SizedBox(width: 6),
              Text('$count/4', style: const TextStyle(color: AppColors.textSecondary, fontSize: 11, fontWeight: FontWeight.w600)),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
                decoration: BoxDecoration(
                  color: (isFull ? AppColors.danger : AppColors.primary).withValues(alpha: 0.12),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(12),
                gradient: isFull ? null : const LinearGradient(colors: [AppColors.primary, AppColors.primaryDark]),
                color: isFull ? AppColors.darkCard : null,
                boxShadow: isFull ? null : [BoxShadow(color: AppColors.primary.withValues(alpha: 0.4), blurRadius: 8, offset: const Offset(0, 3))],
              ),
              child: Text(isFull ? 'Full' : 'Join',
                  style: TextStyle(color: isFull ? AppColors.textMuted : Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
            ),
          ),
        ]),
      ),
