...
import {
  ...
  type PaymentStats, type AdminTransaction,
} from '../../lib/api';
...
function StatCard({
  ...
  return (
    <div
      className="relative rounded-2xl p-5 border border-dark-border overflow-hidden"
...
export default function PaymentManagement() {
  const [tab, setTab]               = useState<'addmoney' | 'withdrawals'>('addmoney');
  ...
  const filtered = (tab === 'addmoney' ? transactions : withdrawals).filter(tx =>
    ...
    <div className="min-h-screen p-6 md:p-8">
      ...
      {toast && (
        <div className={`fixed top-5 right-5 z-50 flex items-center gap-2 px-4 py-3 rounded-xl
                         border text-sm font-medium shadow-lg transition-all
                         ${toast.ok
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Monitor transactions &amp; manage withdrawal requests</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {statsLoading ? (
          ...
          <>
            <StatCard label="Total Revenue"    value={`₹${Number(stats.total_revenue).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
              icon="💰" gradient="from-emerald-500 to-green-700" glow="rgba(34,197,94,0.2)"
              sub={`${stats.total_add_count} transactions`} />
            <StatCard label="Today's Revenue"  value={`₹${Number(stats.today_revenue).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
              icon="📅" gradient="from-blue-500 to-indigo-700" glow="rgba(59,130,246,0.2)" />
            <StatCard label="Pending Withdrawals" value={`₹${Number(stats.pending_amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
            ...
            <StatCard label="Total Withdrawn"  value={`₹${Number(stats.total_withdrawn).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`}
      ...
      <div className="flex gap-1 p-1 rounded-xl border border-dark-border mb-6 w-fit"
        ...
        {([['addmoney', '💸 Add Money'], ['withdrawals', '🏦 Withdrawals']] as const).map(([key, label]) => (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-dark-border">
              {loading ? (
                ...
                <tr>
                  <td colSpan={tab === 'withdrawals' ? 7 : 6}
                ...
                filtered.map((tx, i) => (
                  <tr key={tx.id}
                    className="group hover:bg-dark-border/20 transition-colors"
                    ...
                    <td className="px-5 py-3.5">
                      <span className="text-accent font-medium">🪙 {tx.coins}</span>
                    </td>
        ...
        {filtered.length > 0 && (
          <div className="px-5 py-3 border-t border-dark-border flex items-center justify-between"
            ...
            <p className="text-gray-600 text-xs">
              Showing <span className="text-gray-400 font-medium">{filtered.length}</span> records
            </p>
            {tab === 'addmoney' && stats && (
              <p className="text-gray-600 text-xs">
                Total shown:{' '}
                <span className="text-primary font-semibold">
                  ₹{filtered.reduce((s, t) => s + Number(t.amount), 0).toFixed(2)}
                </span>
              </p>
            )}
          </div>
      ...
      {approveTarget && (
        <ConfirmModal
          ...
          message={`Approve ₹${Number(approveTarget.amount).toFixed(2)} withdrawal for ${approveTarget.username}? This action cannot be undone.`}
